...
export const HeadingElement = ({ element, onChange, onDelete }: HeadingElementProps) => {
  const [error, setError] = useState<string | null>(null);

  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    if (value.length > ELEMENT_CONSTRAINTS.heading.maxLength) {
      setError(`Heading must be ${ELEMENT_CONSTRAINTS.heading.maxLength} characters or less`);
    } else {
      setError(null);
    ...
    <ElementWrapper id={element.id} onDelete={() => onDelete(element.id)}>
      <div className="space-y-2">
        ...
        <p className="text-xs text-gray-400">
          {element.content.length}/{ELEMENT_CONSTRAINTS.heading.maxLength}
        </p>
      </div>
    </ElementWrapper>
  );
