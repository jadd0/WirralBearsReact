...
const RenderElement: React.FC<{ element: BlogElement }> = ({ element }) => {
  switch (element.type) {
    case 'heading':
      return <h2 className="text-3xl font-bold mb-4 mt-8">{element.content}</h2>;
    case 'paragraph':
      return <p className="mb-4 leading-relaxed">{element.content}</p>;
    case 'image':
      return (
        <div className="my-6">
          <img 
          ...
          />
          {element.alt && (
            <p className="text-sm text-center text-gray-500 mt-2">{element.alt}</p>
          )}
        </div>
      );
    default:
