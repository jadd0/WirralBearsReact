...
import { SESSION_ID_LENGTH } from '@/lib/constants';

type NewSession = typeof sessions.$inferInsert;

export const sessionRepository = {
	async createSession(
		sessionDetails: Omit<NewSession, 'id' | 'createdAt' | 'updatedAt'>
	): Promise<boolean> {
	...
	async getFullSchedule(): Promise<FullSessionSchedule> {
		...
		const sessionDaysArr = Array.from(sessionDayMap.values());
		...
		return { sessionDays: sessionDaysArr as FullSessionSchedule['sessionDays'] };
	},
