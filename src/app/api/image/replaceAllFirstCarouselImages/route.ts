...
export async function PUT(request: Request) {
	...
	try {
		...
		const result = await imageServices.replaceAllFirstCarouselImages(
			items as CarouselImageInsert[]
		);

		if (result) {
			return NextResponse.json({
				message: `Successfully replaced first carousel with ${items.length} images`,
			});
		}
		return NextResponse.json(
			{ message: 'Failed to replace first carousel images' },
			{ status: 500 }
		);
	} catch (error) {
		console.error('Error replacing first carousel images:', error);
		return NextResponse.json(
			{
				message: 'Failed to replace first carousel images',
