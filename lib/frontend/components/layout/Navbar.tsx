        ...
        <nav className="bg-[#333] text-base h-[37px] flex items-center justify-center px-7 overflow-hidden">
          {navLinks.map((link) => (
            <Link
              key={link.key}
              href={link.to}
              className="font-extralight px-4 py-2 transition-colors duration-200 text-[15px] text-gray-400 hover:text-white"
            >
              {link.label}
            </Link>
          ))}
            ...
            {navLinks.map((link, index) => {
              ...
              return (
                <Link
                  key={link.key}
                  href={link.to}
                  className={`block w-full text-center py-4 px-6 text-xl font-medium rounded-xl transition-all duration-300 transform hover:scale-105 ${
                    isActive
