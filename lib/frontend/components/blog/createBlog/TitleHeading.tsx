...
import { BLOG_MAX_TITLE_LENGTH } from "@/constants";
import { Input } from "@/components/ui/input";
import { HeadingElement } from "@/shared/types";
...
export const TitleHeadingElement = ({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length > maxLength) {
      setError(
        `${coach ? "Coach name" : "Title"} must be ${maxLength} characters or less`,
      );
      ...
      <div className="space-y-3">
        <Input
          ...
          placeholder={
            coach
              ? "Enter coach name"
              : "Write an engaging title for your blog..."
          }
          maxLength={maxLength}
          required
        />
