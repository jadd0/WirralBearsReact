...
export const BlogEditor = ({
  ...
  // Initialise with a title heading element if not provided
  ...
  const validateAndNotify = (blogElements: BlogElement[]) => {
    try {
      ...
      const result = blogDataSchema.parse(blogData);
            ...
            {contentElements.length === 0 ? (
              <div className="text-center py-10 border-2 border-dashed border-gray-300 rounded-md">
                <p className="text-gray-500">
                  {coach ? "Coach content" : "Your blog"} is empty. Add elements
                  to get started.
                </p>
              </div>
            ) : (
