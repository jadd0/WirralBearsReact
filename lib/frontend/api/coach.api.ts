...
export async function getAllCoachPreviews() {
  return jsonFetch<CoachPreview[]>("/api/coaches");
}

export async function getCoachById(id: string) {
  return jsonFetch<Coaches>(`/api/coaches/${id}`);
}

// TODO: type
export async function createCoach(payload: any) {
  return jsonFetch<Coaches>("/api/coaches", {
    method: "POST",
...
export async function deleteCoach(id: string) {
  await jsonFetch<unknown>(`/api/coaches/${id}`, {
    method: "DELETE",
  });
}
